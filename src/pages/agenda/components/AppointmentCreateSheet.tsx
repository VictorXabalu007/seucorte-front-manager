  ...
  useEffect(() => {
    if (isOpen) {
      form.reset({
        ...
        dateObj: initialData?.dateObj || new Date(),
    ...
    <Sheet open={isOpen} onOpenChange={onOpenChange}>
      <SheetContent side="right" className="bg-card border-border text-foreground sm:max-w-md overflow-y-auto rounded-l-2xl">
        <SheetHeader className="mb-6">
          ...
          <p className="text-sm text-muted-foreground text-left">Preencha os dados para criar um novo agendamento manual.</p>
        </SheetHeader>
          ...
          <form onSubmit={form.handleSubmit(onSave)} className="space-y-5 py-2">
              ...
              <FormField control={form.control} name="serviceId" render={({ field }) => (
                <FormItem>
                  ...
                  <Select onValueChange={field.onChange} value={field.value}>
                    ...
                    <SelectContent className="rounded-xl border-border bg-card">
                      {services.map(s => (
                        <SelectItem key={s.id} value={s.id}>{s.name} ({s.duration}min — R$ {s.price})</SelectItem>
                      ))}
                    </SelectContent>
                  </Select>
                  <FormMessage />
                    ...
                    <PopoverTrigger asChild>
                      <FormControl>
                        <Button
                          variant="outline"
                          className={cn(
                            "w-full justify-start text-left font-normal bg-background/50 border-border rounded-xl h-9",
            ...
            <div className="space-y-3">
              ...
              <FormField control={form.control} name="status" render={({ field }) => (
                <FormItem>
                  <Label className="text-xs text-slate-400 mb-1.5 block">Status do Agendamento</Label>
                  <Select onValueChange={field.onChange} value={field.value}>
                    <FormControl>
                      <SelectTrigger className="bg-background/50 border-border rounded-xl h-9">
